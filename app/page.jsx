...
function getAttendanceFlags(row, rulesInput = DEFAULT_OPERATING_RULES) {
  ...
  const plannedCheckIn = timeTextToMinutes(row?.plannedCheckInTime || row?.plannedCheckIn || DEFAULT_SCHEDULE_CHECK_IN);
  const plannedCheckOut = timeTextToMinutes(row?.plannedCheckOutTime || row?.plannedCheckOut || DEFAULT_SCHEDULE_CHECK_OUT);
...
function buildEffectiveSchedulesForPresence({ schedules = [], students = [], sessions = [], seats = [], defaultSchedule = DEFAULT_SCHEDULE_SETTINGS }) {
  const today = getKstDateString();
  const settings = normalizeDefaultScheduleSettings(defaultSchedule);
  const studentById = {};
  for (const student of students || []) {
    if (student?.id) studentById[student.id] = student;
  }
  ...
  for (const schedule of schedules || []) {
    ...
    explicitStudentIds.add(String(schedule.student_id));
    if (schedule.students?.id && !studentById[schedule.students.id]) studentById[schedule.students.id] = schedule.students;
  }

  const candidateStudents = {};
  const addCandidate = (studentOrId) => {
    const student = typeof studentOrId === 'object' && studentOrId !== null
      ? studentOrId
      : studentById[studentOrId];
    const id = student?.id || studentOrId;
    if (!id) return;
    const resolvedStudent = student || studentById[id] || { id };
    candidateStudents[id] = resolvedStudent;
    if (resolvedStudent?.id && !studentById[resolvedStudent.id]) studentById[resolvedStudent.id] = resolvedStudent;
  };

  for (const schedule of explicitSchedules) addCandidate(schedule.students || schedule.student_id);
  for (const session of sessions || []) addCandidate(session.students || session.student_id);
  for (const seat of seats || []) addCandidate(seat.current_student || seat.current_student_id);
  for (const student of students || []) {
  ...
  const defaultSchedules = Object.values(candidateStudents)
    ...
    .map((student) => ({
      ...
      schedule_date: today,
      planned_check_in: settings.plannedCheckIn,
      planned_check_out: settings.plannedCheckOut,
  ...
  return [...explicitSchedules, ...defaultSchedules];
}
...
입력하지 않고 진행하려면 빈칸으로 확인하거나 취소를 누르세요.`, '');
  ...
  function getTodayScheduleSummary(studentId) {
    ...
    if (!studentId) return ['배정된 학생이 없습니다.'];
    ...
    const breaks = (todayScheduleBreaks || [])
      ...
      .sort((a, b) => String(a.leave_start).localeCompare(String(b.leave_start)));

    const periodLines = getDefaultScheduleSummaryLines(start, end, breaks, defaultSchedule);
    if (!breaks.length) return periodLines;

    const breakLines = breaks.map((item) => {
      const leave = item.leave_start?.slice(0, 5);
      const ret = item.return_time?.slice(0, 5);
      const reason = [item.reason, item.reason_detail].filter(Boolean).join(' · ');
      return leave && ret ? `${leave}~${ret} 외출_${reason || '기타'}` : null;
    }).filter(Boolean);

    return [...periodLines, ...breakLines].sort((a, b) => {
      const aTime = a.slice(0, 5);
      const bTime = b.slice(0, 5);
...
{getTodayScheduleSummary(form.studentId).map((line, index) => (
            <div key={index} className={`today-schedule-line ${line.includes('외출_') ? 'break' : 'study'}`}>
              {line}
            </div>
          ))}
        </PanelSection>
          ...
          <div className="seat-map-wrap">
            <div className={`seat-map seat-map-v4111 seat-map-v4112 ${quickMode ? 'quick-mode' : ''}`}>
              ...
              {seatRows.map((row) => {
                ...
                return (
                  <button key={seat.seat_no} className={`seat ${selectedSeatNo === seat.seat_no ? 'selected' : ''} ${status} ${isFilteredOut ? 'filtered-out' : ''} ${isMissingStudy ? 'study-missing' : ''} ${urgent ? 'priority-attention' : ''} ${mentoringHighlight ? 'mentoring-upcoming' : ''} ${mentorCue && mentoringStatusPriority ? 'mentoring-status-priority' : ''} ${isQuickSelected ? 'quick-selected' : ''} ${quickDisabled ? 'quick-disabled' : ''}`} style={{ left: seat.x, top: seat.y, width: seat.width, height: seat.height }} onClick={() => handleSeatClick(row)}>
    ...
    const breakBlocks = breaks
      .map((item, index) => {
        ...
        return {
          ...
          startMinute: leave,
          endMinute: ret,
          title: `외출_${item.reason || '기타'}`,
          detail: `${minutesToTime(leave)}~${minutesToTime(ret)}${item.reason_detail ? ` · ${item.reason_detail}` : ''}`,
          schedule,
        };
      })
      .filter(Boolean);

    blocks.push(...periodBlocks, ...breakBlocks);
    blocks.sort((a, b) => a.startMinute - b.startMinute || (a.type === 'break-activity' ? 1 : -1));
    if (!blocks.length) blocks.push({ id: `study-${schedule.student_id}-${schedule.schedule_date}-empty`, type: 'self-study', startMinute: start, endMinute: end, title: '자율학습', detail: `${minutesToTime(start)}~${minutesToTime(end)}`, schedule });
    return blocks;
  }

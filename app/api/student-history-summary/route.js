...
async function callOpenAiForSummary({ source, summaryType = 'internal_weekly' }) {
  ...
  const toneGuide = isParent
    ...
    : [
      ...
      '태도, 실행력, 복귀 지연, 인강 과다·비학습 등 개선 포인트를 명확히 지적합니다.',
  ...
  const userPrompt = [
    ...
    '형식:',
    `[${typeTitle} 상담 요약]`,
    '1) 조회기간 핵심 요약 3~5문장 (순공시간 판정과 근거 시간 포함)',
    '2) 출결·체류 패턴 (등하원 시간대, 외출 시간/횟수, 지각·조퇴·결석 특이사항)',
    '3) 학습 태도·집중도 및 순찰 관찰 (과목/학습상태 흐름, 인강 비중, 비학습·수면 신호)',
    '4) 멘토 코멘트 종합',
    '5) 다음 관리 포인트 3가지',
    '',
    '작성 규칙:',
    ...
    '- 순공시간·등하원 시간·외출(시간/횟수)·순찰 기록·멘토 코멘트를 서로 연결해 종합적으로 해석합니다. 단순 나열이 아니라 패턴과 원인 중심으로 엮습니다.',
    '- 순찰(study) 기록에서 인강(인터넷 강의) 비중이 과도하게 높으면 반드시 짚어줍니다. 인강만 듣는 것은 자기주도 학습이 아니라 수업 청강에 가까우므로, 문제풀이·복습 등 능동적 자기공부로 전환하도록 제안합니다.',
    '- 플래너 제출 여부/횟수는 판단 근거로 사용하지 않습니다. (사진 판독 없이 제출 횟수만으로 실행력을 단정하지 않습니다.)',
    '',
    '순공시간 운영 기준:',

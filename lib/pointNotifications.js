...
function buildPenaltyPayload({ student, stage, stageThresholdLabel, penaltyState, recentRows, baseDate }) {
  ...
  // v41-256: 기준 점수는 설정에서 바뀝니다. stage(10/20/30)는 단계 번호일 뿐이라
  // 문구에 그대로 쓰면 '10점 초과'처럼 실제와 다른 숫자가 나갑니다.
  // 승인된 템플릿의 #{안내단계} 변수 값만 달라지므로 재검수는 필요 없습니다.
  const thresholdText = stageThresholdLabel || `${stage}점 초과`;
  ...
  const messageText = [
    ...
    `- 순벌점: ${penaltyState?.penaltyNet ?? 0}점 (${thresholdText} · ${stageLabel})`,
    '',
    guide,
    '',
    '최근 상벌점 내역',
...
export async function sendPointNotification({
  ...
  const built = kind === 'penalty'
    ? buildPenaltyPayload({ student, stage, stageThresholdLabel, penaltyState, recentRows, baseDate })

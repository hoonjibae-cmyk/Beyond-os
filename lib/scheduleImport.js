...
import { parseSpecialSchedule } from './specialScheduleParse.js';
...
export function guessColumnMapping(headers = []) {
  ...
  return { nameColumn: nameColumn ? nameColumn.index : null, days, specialColumn: guessSpecialColumn(list) };
}

/**
 * "특별 일정 상세"처럼 날짜 지정 예외를 적은 자유 서술 칸을 찾습니다.
 * "특별 일정이 있나요?" 같은 예/아니오 칸은 제외합니다.
 */
function guessSpecialColumn(list = []) {
  let best = 0;
  let column = null;
  for (const item of list) {
    const compact = stripSpace(item.header);
    if (!compact) continue;
    // 예/아니오 질문은 날짜 정보가 없습니다.
    if (/있나요|있으신|여부|하시나요/.test(compact)) continue;
    let score = 0;
    if (compact.includes('특별') && compact.includes('상세')) score = 3;
    else if (compact.includes('특별') && compact.includes('일정')) score = 2;
    else if (compact.includes('예외') && compact.includes('일정')) score = 2;
    if (score > best) { best = score; column = item.index; }
  }
  return column;
}
 ...
 */
export function buildWeeklyPatterns(rows = [], mapping = {}, baseByDay = {}, options = {}) {
  ...
  for (let rowIndex = 0; rowIndex < (rows || []).length; rowIndex += 1) {
    ...
    // 특정 날짜 예외(가족여행·병원 등)는 요일 패턴과 별도로 해석합니다.
    const specialColumn = mapping.specialColumn;
    const special = (specialColumn === null || specialColumn === undefined)
      ? { items: [], needsReview: false, raw: '' }
      : parseSpecialSchedule(pick(row, specialColumn), { periodStart, periodEnd });
    if (special.needsReview) {
      issues.push('특별일정: 날짜나 시각이 분명하지 않은 항목이 있습니다.');
    }

    results.push({ rowIndex, name, days, issues, filledDays, reviewDays, special });
  }
  return results;
}
  ...
  while (cursor <= endDate && guard < maxDays) {
    ...
    if (config && !config.absent && !config.unknown && (config.checkIn || config.checkOut)) {
      rows.push({
        ...
        // v41-199: 늦은 등원 / 이른 하원 사유. 반영할 때 일정 메모로 남깁니다.
...
/**
 * 브라우저에서 만들어 보낸 특별 일정 적용값을 서버에서 다시 검증합니다.
 * 형식이 맞지 않거나 기간을 벗어난 항목은 조용히 버립니다.
 */
export function normalizeSpecialOverrides(value = {}, { periodStart = '', periodEnd = '' } = {}) {
  const inPeriod = (iso) => {
  ...
  const startFromRaw = String(value?.startFrom || '');
  const startFrom = ISO_DATE.test(startFromRaw) ? startFromRaw : '';

  const absent = [];
  const seen = new Set();
  for (const item of Array.isArray(value?.absent) ? value.absent : []) {
    const date = String(item?.date || '');
    if (!inPeriod(date) || seen.has(date)) continue;
    seen.add(date);
    absent.push({ date, reason: String(item?.reason || '').slice(0, 100) });
  }

  const away = [];
  for (const item of Array.isArray(value?.away) ? value.away : []) {
    const date = String(item?.date || '');
    const start = String(item?.start || '');
    if (!inPeriod(date) || !HHMM.test(start)) continue;
    const end = HHMM.test(String(item?.end || '')) ? String(item.end) : '';
    away.push({ date, start, end, reason: String(item?.reason || '').slice(0, 100) });
    if (away.length >= 200) break;
  }
  ...
  return { startFrom, absent, away, notes };
}

/**
 * 요일 패턴으로 펼친 날짜 목록에 "특정 날짜 예외"를 덮어씌웁니다.
 *
 *   startFrom : 그 날 이전 날짜는 아예 만들지 않습니다. ("7/22부터 등원")
 *   absent    : 그 날은 결석으로 표시합니다. (외출은 지웁니다)
 *   away      : 그 날 외출을 추가합니다. (요일 패턴의 정기 외출과 함께 남습니다)
 *
 * @param {Array} rows expandPatternToDates 결과
 * @param {Object} overrides buildSpecialOverrides 결과
 */
export function applySpecialToDates(rows = [], overrides = {}) {
  const startFrom = overrides?.startFrom || '';
  const absentByDate = {};
  for (const item of overrides?.absent || []) {
    if (item?.date) absentByDate[item.date] = String(item.reason || '');
  }
  const awayByDate = {};
  for (const item of overrides?.away || []) {
    if (!item?.date || !item.start) continue;
    if (!awayByDate[item.date]) awayByDate[item.date] = [];
    awayByDate[item.date].push({ start: item.start, end: item.end || '', reason: item.reason || '' });
  }

  const out = [];
  for (const row of rows || []) {
    if (startFrom && row.date < startFrom) continue;
    if (Object.prototype.hasOwnProperty.call(absentByDate, row.date)) {
      out.push({ ...row, absent: true, absentReason: absentByDate[row.date], breaks: [] });
      continue;
    }
    const extra = awayByDate[row.date];
    if (extra && extra.length) {
      out.push({ ...row, breaks: [...(row.breaks || []), ...extra] });
      continue;
    }
    out.push(row);
  }

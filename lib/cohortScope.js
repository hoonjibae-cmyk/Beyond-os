...
/**
 * 기수의 기간(start ~ end)까지 함께 돌려줍니다.
 *
 * 상벌점처럼 "이 기수 기간의 기록만 센다"가 필요한 곳에서 씁니다.
 * 기수를 아직 만들지 않았거나 기간이 비어 있으면 null 입니다.
 * 그 경우 호출한 쪽은 예전처럼 전체 기간을 봅니다.
 */
export async function loadCohortRange(supabase, requested, today) {
  try {
    const cohort = await resolveScopeCohort(supabase, requested, today);

...
import { getNoticeCategory } from './noticeTemplates';
import { SCHEDULE_CONFIRM_TEMPLATE } from './scheduleConfirmTemplate';
...
function normalizeReportType(value) {
  ...
  // v41-161: 상벌점 안내(벌점 단계 / 상품 지급)
  if (value === 'point_penalty') return 'point_penalty';
  if (value === 'point_reward') return 'point_reward';
...
function getTemplateEnv(reportType, noticeCategory) {
  ...
  if (type === 'point_penalty') return envStatus(['SOLAPI_TEMPLATE_ID_POINT_PENALTY']);
  if (type === 'point_reward') return envStatus(['SOLAPI_TEMPLATE_ID_POINT_REWARD']);
  if (type === 'schedule_confirm') return envStatus([...SCHEDULE_CONFIRM_TEMPLATE.templateIdEnvs]);
  if (type === 'notice') {
    const cat = getNoticeCategory(noticeCategory);
    return envStatus([...(cat.templateIdEnvs || []), 'SOLAPI_TEMPLATE_ID_NOTICE']);
  }
  return envStatus(['SOLAPI_TEMPLATE_ID_DAILY']);
}

export function getSolapiConfig(reportType = 'daily', noticeCategory) {
  ...
  const template = getTemplateEnv(reportType, noticeCategory);
...
export function getSolapiAdapterStatus(reportType = 'daily', noticeCategory) {
  const type = normalizeReportType(reportType);
  const config = getSolapiConfig(type, noticeCategory);
  return {
    provider: 'solapi',
    reportType: type,
...
export function buildSolapiRequestBody(payload = {}, config = getSolapiConfig(payload.reportType, payload.noticeCategory)) {
...
export async function sendSolapiAlimtalk(payload = {}, options = {}) {
  const reportType = normalizeReportType(payload.reportType);
  const config = options.config || getSolapiConfig(reportType, payload.noticeCategory);
  const status = getSolapiAdapterStatus(reportType, payload.noticeCategory);

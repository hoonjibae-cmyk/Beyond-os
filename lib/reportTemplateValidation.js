...
export const REQUIRED_TEMPLATE_VARIABLES = {
  ...
  notice: ['#{링크}'],
  // v41-161: 상벌점 안내
  point_penalty: ['#{학생명}', '#{기준일}', '#{상점}', '#{벌점}', '#{순벌점}', '#{안내단계}', '#{안내문구}', '#{최근내역}'],
  // v41-214: #{지급안내} 는 검수 반려로 본문 고정 문구가 되었습니다.
  point_reward: ['#{학생명}', '#{기준일}', '#{상점}', '#{벌점}', '#{순점수}', '#{최근내역}'],
  // v41-203: 학부모 시간표 확인 링크
  schedule_confirm: ['#{학생명}', '#{기간}', '#{확인링크}'],
};

function resolveNoticeCategory(payload = {}) {
  const templateVariables = payload.templateVariables || {};
  return payload.noticeCategory || templateVariables.noticeCategory;
}
...
export function getKakaoVariables(payload = {}, reportType = 'daily') {
  ...
  if (type === 'schedule_confirm') {
    return {
      '#{학생명}': templateVariables.studentName || payload.studentName || '',
      '#{기간}': templateVariables.period || [payload.startDate, payload.endDate].filter(Boolean).join(' ~ '),
      '#{확인링크}': templateVariables.confirmLink || templateVariables.reportLink || payload.reportLink || '',
    };
  }

  if (type === 'notice') {
    const cat = getNoticeCategory(resolveNoticeCategory(payload));
    if (cat.input === 'fields') {
      const data = templateVariables.noticeData || payload.noticeData || {};
      const out = {};
      for (const f of cat.fields) out[f.kakao] = String(data[f.key] ?? templateVariables[f.key] ?? '');
      return out;
    }
    return {
      '#{링크}': templateVariables.noticeLink || templateVariables.reportLink || payload.reportLink || '',
      '#{공지제목}': templateVariables.noticeTitle || payload.noticeTitle || '',
    };
  }
...
export function validateKakaoTemplateVariables(payload = {}, reportType = 'daily') {
  const type = normalizeReportType(reportType);
  const required = type === 'notice'
    ? getNoticeCategory(resolveNoticeCategory(payload)).required
    : REQUIRED_TEMPLATE_VARIABLES[type];
  ...
  if (type === 'schedule_confirm') {
    const confirmLink = String(variables['#{확인링크}'] || '');
    if (confirmLink && !/^https?:\/\//i.test(confirmLink)) {
      warnings.push('#{확인링크}가 http/https URL 형식이 아닙니다.');
    }

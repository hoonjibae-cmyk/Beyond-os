...
// 학생 연락처(students.student_phone)가 등록된 학생에게만 발송되며, 없으면 자동으로 건너뜁니다.
export const DEFAULT_STUDENT_RECIPIENT_SETTINGS = {
  dailyEnabled: false,
  weeklyEnabled: false,
};
...
export const DEFAULT_REPORT_SEND_SETTINGS = {
  ...
  studentRecipients: DEFAULT_STUDENT_RECIPIENT_SETTINGS,
};
...
export function normalizeStudentRecipientSettings(value = {}) {
  const source = value && typeof value === 'object' ? value : {};
  const fallback = DEFAULT_STUDENT_RECIPIENT_SETTINGS;
  return {
    dailyEnabled: normalizeBoolean(source.dailyEnabled ?? source.daily_enabled, fallback.dailyEnabled),
    weeklyEnabled: normalizeBoolean(source.weeklyEnabled ?? source.weekly_enabled, fallback.weeklyEnabled),
  };
}

// 리포트 종류에 따라 '학생 본인 발송'이 켜져 있는지 확인합니다.
export function isStudentRecipientEnabled(reportType = 'daily', settings = DEFAULT_REPORT_SEND_SETTINGS) {
  const normalized = normalizeStudentRecipientSettings(settings?.studentRecipients ?? settings?.student_recipients);
  return reportType === 'weekly' ? normalized.weeklyEnabled : normalized.dailyEnabled;
}
...
export function normalizeReportSendSettings(value = {}) {
  ...
  return {
    ...
    studentRecipients: normalizeStudentRecipientSettings(source.studentRecipients ?? source.student_recipients ?? DEFAULT_STUDENT_RECIPIENT_SETTINGS),
  };
}

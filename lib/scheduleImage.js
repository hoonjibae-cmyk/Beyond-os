...
//
// v41-198: 기준 시간표(baseByDay)가 없어도 일정 메모는 그대로 보여 줍니다.
// 늦은 등원/이른 하원 판정이 안 되더라도 "왜 이 시간인지"는 학부모가 알아야 합니다.
...
export async function renderScheduleImage(entry, meta = {}) {
  ...
  for (const dayKey of DAY_ORDER) {
    ...
    const isRest = !day;
    // v41-198: 매주 빠지는 요일은 특별 일정에 날짜를 늘어놓지 않고 이 칸에 사유와 함께 적습니다.
    const isAbsent = Boolean(day?.absent);
    ...
    ctx.fillStyle = isAbsent ? COLORS.absentInk : (isRest ? COLORS.restInk : COLORS.ink);
    ...
    if (isRest) {
      ...
      ctx.fillStyle = COLORS.absentInk;
      if (reason) {
        ctx.font = font(15, 700);
        ctx.fillText('등원 없음 (결석)', tableLeft + colDay + 16, midY - 10);
        ctx.font = font(11.5, 600);
        ctx.fillText(clip(ctx, reason, colTime - 26), tableLeft + colDay + 16, midY + 11);
      } else {
        ctx.font = font(15, 700);
        ctx.fillText('등원 없음 (결석)', tableLeft + colDay + 16, midY);
      }
    } else if (shift) {

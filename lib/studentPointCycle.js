...
export function resolvePenaltyStages(pointRows = [], actionRows = [], options = {}) {
  ...
  for (const def of PENALTY_STAGES) {
    // "초과" 기준: 10점 단계는 순벌점 11점부터입니다.
    if (penaltyNet <= def.stage) continue;
    ...
    const item = {
      ...
      memo: record?.memo || '',
      message: `순벌점 ${def.stage}점 초과 — ${def.label} 대상입니다`,
    };
  ...
  const nextStage = PENALTY_STAGES.find((def) => penaltyNet <= def.stage) || null;
  ...
  return {
    ...
    remainingToNextStage: nextStage ? Math.max(0, nextStage.stage + 1 - penaltyNet) : null,
  };
}
